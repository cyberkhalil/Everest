...
public interface JdbcConnection extends java.sql.Connection, MysqlConnection, TransactionEventHandler {
     ...
     *
     * @return true if this connection currently a participant in an XA
     * transaction
     */
    ...
    /**
     * Resets the server-side state of this connection. Doesn't work if
     * isParanoid() is set (it will become a no-op in this case). Usually only
     * used from connection pooling code.
     *
    ...
    /**
     * Sets the sql select limit max-rows for this session if different from
     * current.
     *
     ...
     * @param skipLocalTeardown if true, driver tries to close connection
     * normally, performing rollbacks, closing open statements etc; otherwise
     * the force close is performed
