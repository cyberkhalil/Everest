 ...
 *
 * For those looking further into the driver implementation, it is not an API that is used for
...
public interface JdbcConnection extends java.sql.Connection, MysqlConnection, TransactionEventHandler {
    ...
    /**
     * Changes the user on this connection by performing a re-authentication. If authentication
     * fails, the connection is failed.
     *
    ...
    /**
     * Reports how long this connection has been idle. This time (reported in milliseconds) is
    ...
    /**
     * Has this connection tried to execute a query on the "master" server (first host in a multiple
     * host list).
     *
    ...
    /**
     * Is this connection connected to the first host in the list if there is a list of servers in
     * the URL?
     *
    ...
    /**
     * Returns the -session- value of 'auto_increment_increment' from the server if it exists, or
     * '1' if not.
     *
     ...
     * _exactly_.
     *
     * This method is synchronized by the caller on getMutex(), so if calling this method from
     * internal code in the driver, make sure it's synchronized on the mutex that guards
    ...
    /**
     * Caches CachedResultSetMetaData that has been placed in the cache using the given SQL as a
     * key.
     *
     * This method is synchronized by the caller on getMutex(), so if calling this method from
     * internal code in the driver, make sure it's synchronized on the mutex that guards
