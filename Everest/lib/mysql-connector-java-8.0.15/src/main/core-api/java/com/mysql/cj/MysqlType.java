...
public enum MysqlType implements SQLType {
     ...
     * number. M is the total number of digits (the precision) and D is the
     * number of digits after the decimal point (the scale). The decimal point
     * and (for negative numbers) the "-" sign are not counted in M. If D is 0,
     * values have no decimal point or fractional part. The maximum number of
     * digits (M) for DECIMAL is 65. The maximum number of supported decimals
     * (D) is 30. If D is omitted, the default is 0. If M is omitted, the
     * default is 10.
     *
     * Protocol: FIELD_TYPE_DECIMAL = 0 Protocol: FIELD_TYPE_NEWDECIMAL = 246
     *
     * These types are synonyms for DECIMAL: DEC[(M[,D])] [UNSIGNED] [ZEROFILL],
     * NUMERIC[(M[,D])] [UNSIGNED] [ZEROFILL], FIXED[(M[,D])] [UNSIGNED]
     * [ZEROFILL]
     */
    ...
    /**
     * TINYINT[(M)] [UNSIGNED] [ZEROFILL] A very small integer. The signed range
     * is -128 to 127. The unsigned range is 0 to 255.
     *
     ...
     *
     * BOOLEAN is converted to TINYINT(1) during DDL execution i.e. it has the
     * same precision=3. Thus we have to look at full data type name and convert
     * TINYINT to BOOLEAN (or BIT) if it has "(1)" length specification.
     *
     * Protocol: FIELD_TYPE_TINY = 1
     */
     ...
     * floating-point number. Permissible values are -3.402823466E+38 to
     * -1.175494351E-38, 0, and 1.175494351E-38 to 3.402823466E+38. These are
     * the theoretical limits, based on the IEEE standard. The actual range
     ...
     *
     * Additionally: FLOAT(p) [UNSIGNED] [ZEROFILL] A floating-point number. p
     ...
     * p is from 0 to 24, the data type becomes FLOAT with no M or D values. If
     * p is from 25 to 53, the data type becomes DOUBLE with no M or D values.
     * The range of the resulting column is the same as for the single-precision
     * FLOAT or double-precision DOUBLE data types.
     */
    ...
    /**
     * DOUBLE[(M,D)] [UNSIGNED] [ZEROFILL] A normal-size (double-precision)
     * floating-point number. Permissible values are -1.7976931348623157E+308 to
     * -2.2250738585072014E-308, 0, and 2.2250738585072014E-308 to
     * 1.7976931348623157E+308. These are the theoretical limits, based on the
     * IEEE standard. The actual range might be slightly smaller depending on
     ...
     *
     * These types are synonyms for DOUBLE: DOUBLE PRECISION[(M,D)] [UNSIGNED]
     * [ZEROFILL], REAL[(M,D)] [UNSIGNED] [ZEROFILL]. Exception: If the
     * REAL_AS_FLOAT SQL mode is enabled, REAL is a synonym for FLOAT rather
     * than DOUBLE.
     */
    ...
    /**
     * TIMESTAMP[(fsp)] A timestamp. The range is '1970-01-01 00:00:01.000000'
     * UTC to '2038-01-19 03:14:07.999999' UTC. TIMESTAMP values are stored as
     * the number of seconds since the epoch ('1970-01-01 00:00:00' UTC). A
     * TIMESTAMP cannot represent the value '1970-01-01 00:00:00' because that
     * is equivalent to 0 seconds from the epoch and the value 0 is reserved for
     * representing '0000-00-00 00:00:00', the "zero" TIMESTAMP value. An
     * optional fsp value in the range from 0 to 6 may be given to specify
    ...
    BIGINT_UNSIGNED("BIGINT UNSIGNED", Types.BIGINT, BigInteger.class, MysqlType.FIELD_FLAG_UNSIGNED | MysqlType.FIELD_FLAG_ZEROFILL, MysqlType.IS_DECIMAL, 20L,
            "[(M)] [UNSIGNED] [ZEROFILL]"),
    /**
     * MEDIUMINT[(M)] [UNSIGNED] [ZEROFILL] A medium-sized integer. The signed
     * range is -8388608 to 8388607. The unsigned range is 0 to 16777215.
     *
     ...
     * '838:59:59.000000'. MySQL displays TIME values in 'HH:MM:SS[.fraction]'
     * format, but permits assignment of values to TIME columns using either
     * strings or numbers. An optional fsp value in the range from 0 to 6 may be
     * given to specify fractional seconds precision. A value of 0 signifies
     ...
     * '1000-01-01 00:00:00.000000' to '9999-12-31 23:59:59.999999'. MySQL
     ...
     * numbers. An optional fsp value in the range from 0 to 6 may be given to
    ...
    /**
     * YEAR[(4)] A year in four-digit format. MySQL displays YEAR values in YYYY
     * format, but permits assignment of values to YEAR columns using either
     * strings or numbers. Values display as 1901 to 2155, and 0000. Protocol:
     * FIELD_TYPE_YEAR = 13
     */
    YEAR("YEAR", Types.DATE, Date.class, 0, MysqlType.IS_NOT_DECIMAL, 4L, "[(4)]"),
    /**
     * [NATIONAL] VARCHAR(M) [CHARACTER SET charset_name] [COLLATE
     * collation_name] A variable-length string. M represents the maximum column
     * length in characters. The range of M is 0 to 65,535. The effective
     * maximum length of a VARCHAR is subject to the maximum row size (65,535
     * bytes, which is shared among all columns) and the character set used. For
     * example, utf8 characters can require up to three bytes per character, so
     * a VARCHAR column that uses the utf8 character set can be declared to be a
     ...
     * character set. NVARCHAR is shorthand for NATIONAL VARCHAR.
     *
     * Protocol: FIELD_TYPE_VARCHAR = 15 Protocol: FIELD_TYPE_VAR_STRING = 253
     */
    ...
    /**
     * VARBINARY(M) The VARBINARY type is similar to the VARCHAR type, but
     * stores binary byte strings rather than nonbinary character strings. M
     ...
     * 1 to 64. The default is 1 if M is omitted. Protocol: FIELD_TYPE_BIT = 16
     */
     ...
     * of the max_allowed_packet system variable (max value 1073741824). (While
     * the server manipulates a JSON value internally in memory, it can be
     ...
     * collation_name] An enumeration. A string object that can have only one
     * value, chosen from the list of values 'value1', 'value2', ..., NULL or
     * the special '' error value. ENUM values are represented internally as
     * integers. An ENUM column can have a maximum of 65,535 distinct elements.
     * (The practical limit is less than 3000.) A table can have no more than
     ...
     * collation_name] A set. A string object that can have zero or more values,
     * each of which must be chosen from the list of values 'value1', 'value2',
     * ... SET values are represented internally as integers. A SET column can
     * have a maximum of 64 distinct members. A table can have no more than 255
    ...
    /**
     * TINYBLOB A BLOB column with a maximum length of 255 (28 - 1) bytes. Each
     * TINYBLOB value is stored using a 1-byte length prefix that indicates the
    ...
    /**
     * TINYTEXT [CHARACTER SET charset_name] [COLLATE collation_name] A TEXT
     * column with a maximum length of 255 (28 - 1) characters. The effective
     * maximum length is less if the value contains multibyte characters. Each
     * TINYTEXT value is stored using a 1-byte length prefix that indicates the
     * number of bytes in the value.
     *
     * Protocol:FIELD_TYPE_TINY_BLOB = 249
     */
    ...
    /**
     * MEDIUMBLOB A BLOB column with a maximum length of 16,777,215 (224 - 1)
     * bytes. Each MEDIUMBLOB value is stored using a 3-byte length prefix that
    ...
    /**
     * MEDIUMTEXT [CHARACTER SET charset_name] [COLLATE collation_name] A TEXT
     * column with a maximum length of 16,777,215 (224 - 1) characters. The
    ...
    /**
     * LONGBLOB A BLOB column with a maximum length of 4,294,967,295 or 4GB (232
     * - 1) bytes. The effective maximum length of LONGBLOB columns depends on
    ...
    /**
     * LONGTEXT [CHARACTER SET charset_name] [COLLATE collation_name] A TEXT
     * column with a maximum length of 4,294,967,295 or 4GB (232 - 1)
     ...
     * Each BLOB value is stored using a 2-byte length prefix that indicates the
     * number of bytes in the value. An optional length M can be given for this
     * type. If this is done, MySQL creates the column as the smallest BLOB type
    ...
    /**
     * TEXT[(M)] [CHARACTER SET charset_name] [COLLATE collation_name] A TEXT
     * column with a maximum length of 65,535 (216 - 1) characters. The
     * effective maximum length is less if the value contains multibyte
     * characters. Each TEXT value is stored using a 2-byte length prefix that
     * indicates the number of bytes in the value. An optional length M can be
     * given for this type. If this is done, MySQL creates the column as the
     ...
     * collation_name] A fixed-length string that is always right-padded with
     * spaces to the specified length when stored. M represents the column
     * length in characters. The range of M is 0 to 255. If M is omitted, the
     * length is 1. Note Trailing spaces are removed when CHAR values are
     * retrieved unless the PAD_CHAR_TO_FULL_LENGTH SQL mode is enabled. CHAR is
     * shorthand for CHARACTER. NATIONAL CHAR (or its equivalent short form,
     * NCHAR) is the standard SQL way to define that a CHAR column should use
     * some predefined character set. MySQL 4.1 and up uses utf8 as this
     * predefined character set.
     *
     * MySQL permits you to create a column of type CHAR(0). This is useful
     * primarily when you have to be compliant with old applications that depend
     * on the existence of a column but that do not actually use its value.
     * CHAR(0) is also quite nice when you need a column that can take only two
     * values: A column that is defined as CHAR(0) NULL occupies only one bit
     * and can take only the values NULL and '' (the empty string).
     *
    ...
    /**
     * BINARY(M) The BINARY type is similar to the CHAR type, but stores binary
    ...
    /**
     * Get MysqlType matching the full MySQL type name, for example
     * "DECIMAL(5,3) UNSIGNED ZEROFILL". Distinct *_UNSIGNED type will be
     * returned if "UNSIGNED" is present in fullMysqlTypeName.
     *
     ...
     * @param isDec isDec
     * @param precision represents the maximum column size that the server
     ...
     * String representation (assuming the maximum allowed precision of the
