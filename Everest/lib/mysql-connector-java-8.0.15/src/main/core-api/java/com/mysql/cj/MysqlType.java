...
public enum MysqlType implements SQLType {
     ...
     * scale). The decimal point and (for negative numbers) the "-" sign are not counted in M. If D
     * is 0, values have no decimal point or fractional part. The maximum number of digits (M) for
     * DECIMAL is 65. The maximum number of supported decimals (D) is 30. If D is omitted, the
     * default is 0. If M is omitted, the default is 10.
     *
     * Protocol: FIELD_TYPE_DECIMAL = 0 Protocol: FIELD_TYPE_NEWDECIMAL = 246
     *
     * These types are synonyms for DECIMAL: DEC[(M[,D])] [UNSIGNED] [ZEROFILL], NUMERIC[(M[,D])]
     ...
     * 3.402823466E+38. These are the theoretical limits, based on the IEEE standard. The actual
     ...
     * M and D are omitted, values are stored to the limits permitted by the hardware. A
     * single-precision floating-point number is accurate to approximately 7 decimal places.
     *
     * Protocol: FIELD_TYPE_FLOAT = 4
     *
     * Additionally: FLOAT(p) [UNSIGNED] [ZEROFILL] A floating-point number. p represents the
     ...
     * values. If p is from 25 to 53, the data type becomes DOUBLE with no M or D values. The range
     * of the resulting column is the same as for the single-precision FLOAT or double-precision
     * DOUBLE data types.
     */
     ...
     * 2.2250738585072014E-308 to 1.7976931348623157E+308. These are the theoretical limits, based
     * on the IEEE standard. The actual range might be slightly smaller depending on your hardware
     * or operating system.
     *
     * M is the total number of digits and D is the number of digits following the decimal point. If
     * M and D are omitted, values are stored to the limits permitted by the hardware. A
     ...
     *
     * These types are synonyms for DOUBLE: DOUBLE PRECISION[(M,D)] [UNSIGNED] [ZEROFILL],
     * REAL[(M,D)] [UNSIGNED] [ZEROFILL]. Exception: If the REAL_AS_FLOAT SQL mode is enabled, REAL
     ...
     * range from 0 to 6 may be given to specify fractional seconds precision. A value of 0
     * signifies that there is no fractional part. If omitted, the default precision is 0.
     *
    ...
    /**
     * BIGINT[(M)] [UNSIGNED] [ZEROFILL] A large integer. The signed range is -9223372036854775808
    ...
    /**
     * DATE A date. The supported range is '1000-01-01' to '9999-12-31'. MySQL displays DATE values
    ...
    /**
     * TIME[(fsp)] A time. The range is '-838:59:59.000000' to '838:59:59.000000'. MySQL displays
     * TIME values in 'HH:MM:SS[.fraction]' format, but permits assignment of values to TIME columns
     * using either strings or numbers. An optional fsp value in the range from 0 to 6 may be given
     * to specify fractional seconds precision. A value of 0 signifies that there is no fractional
     * part. If omitted, the default precision is 0.
     *
     ...
     * 00:00:00.000000' to '9999-12-31 23:59:59.999999'. MySQL displays DATETIME values in
     ...
     * assignment of values to YEAR columns using either strings or numbers. Values display as 1901
     * to 2155, and 0000. Protocol: FIELD_TYPE_YEAR = 13
     */
     ...
     * which is shared among all columns) and the character set used. For example, utf8 characters
     * can require up to three bytes per character, so a VARCHAR column that uses the utf8 character
     ...
     *
     * Note MySQL 5.7 follows the standard SQL specification, and does not remove trailing spaces
     * from VARCHAR values.
     *
    ...
    /**
     * VARBINARY(M) The VARBINARY type is similar to the VARCHAR type, but stores binary byte
     * strings rather than nonbinary character strings. M represents the maximum column length in
     * bytes.
     *
     * Protocol: FIELD_TYPE_VARCHAR = 15 Protocol: FIELD_TYPE_VAR_STRING = 253
     */
    ...
    /**
     * BIT[(M)] A bit-field type. M indicates the number of bits per value, from 1 to 64. The
     * default is 1 if M is omitted. Protocol: FIELD_TYPE_BIT = 16
     */
     ...
     * max_allowed_packet system variable (max value 1073741824). (While the server manipulates a
     * JSON value internally in memory, it can be larger; the limit applies when the server stores
     * it.)
     *
     ...
     * string object that can have zero or more values, each of which must be chosen from the list
     * of values 'value1', 'value2', ... SET values are represented internally as integers. A SET
     * column can have a maximum of 64 distinct members. A table can have no more than 255 unique
     ...
     * multibyte characters. Each TINYTEXT value is stored using a 1-byte length prefix that
     ...
     * the value contains multibyte characters. The effective maximum length of LONGTEXT columns
     * also depends on the configured maximum packet size in the client/server protocol and
     * available memory. Each LONGTEXT value is stored using a 4-byte length prefix that indicates
     ...
     * optional length M can be given for this type. If this is done, MySQL creates the column as
     ...
     * indicates the number of bytes in the value. An optional length M can be given for this type.
     * If this is done, MySQL creates the column as the smallest TEXT type large enough to hold
     ...
     * PAD_CHAR_TO_FULL_LENGTH SQL mode is enabled. CHAR is shorthand for CHARACTER. NATIONAL CHAR
     * (or its equivalent short form, NCHAR) is the standard SQL way to define that a CHAR column
     * should use some predefined character set. MySQL 4.1 and up uses utf8 as this predefined
     * character set.
     *
     * MySQL permits you to create a column of type CHAR(0). This is useful primarily when you have
     * to be compliant with old applications that depend on the existence of a column but that do
     * not actually use its value. CHAR(0) is also quite nice when you need a column that can take
     * only two values: A column that is defined as CHAR(0) NULL occupies only one bit and can take
     * only the values NULL and '' (the empty string).
     *
    ...
    /**
     * Fall-back type for those MySQL data types which c/J can't recognize. Handled the same as
     * BLOB.
     *
     * Has no protocol ID.
     */
     ...
     * ZEROFILL". Distinct *_UNSIGNED type will be returned if "UNSIGNED" is present in
     * fullMysqlTypeName.
     *
