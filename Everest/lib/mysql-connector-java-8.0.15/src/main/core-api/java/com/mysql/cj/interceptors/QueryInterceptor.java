 ...
 *
 * QueryInterceptors are "chainable" when configured by the user, the results
 * returned by the "current" interceptor will be passed on to the next on in the
 * chain, from left-to-right order, as specified by the user in the driver
 * configuration property "queryInterceptors".
 */
public interface QueryInterceptor {
     ...
     * <strong>must</strong> be passed via setURL() on the DataSource.
     * QueryInterceptor properties are not exposed via accessor/mutator methods
     * on DataSources.
     * @param log logger
    ...
    /**
     * Called before the given query is going to be sent to the server for
     * processing.
     *
     * Interceptors are free to return a result set (which must implement the
     * interface {@link Resultset}), and if so, the server will not execute the
     * query, and the given result set will be returned to the application
     * instead.
     *
     ...
     * @param interceptedQuery the actual {@link Query} instance being
     * intercepted
     * @param <T> {@link Resultset} object
     *
     * @return a {@link Resultset} that should be returned to the application
     * instead of results that are created from actual execution of the
     * intercepted query.
     */
    <T extends Resultset> T preProcess(Supplier<String> sql, Query interceptedQuery);

    /**
     * Called before the given query packet is going to be sent to the server
     * for processing.
     *
     * Interceptors are free to return a PacketPayload, and if so, the server
     * will not execute the query, and the given PacketPayload will be returned
     ...
     * @param serverSession {@link ServerSession} object after the query
     * execution
     * @param <T> {@link Resultset} object
     *
     * @return a {@link Resultset} that should be returned to the application
    ...
    /**
     * Called after the given query packet has been sent to the server for
     * processing.
     *
     * Interceptors are free to return either a different PacketPayload than the
